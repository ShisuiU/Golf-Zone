...
async function FeedPosts({
  ...
  cursor?: number;
  /** Se termine par une barre : « / » ou « /fil/ ». */
  base: string;
}) {
  ...
  return (
    <>
      ...
      {page?.more ? (
        <Link
          href={`${base}?avant=${posts[posts.length - 1].id}`}
...
/**
 * Le fil, tel qu'il s'affiche sur la page d'accueil d'un membre et sur `/fil`,
 * où un visiteur peut le parcourir sans compte. `base` sert aux liens de
 * pagination : ils doivent revenir sur la route d'où l'on vient.
 */
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden">
      ...
      <main id="contenu" tabIndex={-1} className="relative mx-auto w-full max-w-[1004px] flex flex-1 flex-col px-4 py-6 lg:py-10">
        ...
        <h1 className="sr-only">Le fil de Zone Golf</h1>
        ...
        <div className="mx-auto w-full max-w-[648px] lg:mx-0 lg:max-w-none lg:grid lg:grid-cols-[minmax(0,648px)_296px] lg:justify-center lg:gap-7">
          <div className="flex min-w-0 flex-col gap-5">
            ...
            {cursor ? (
              <Link href={base} className="py-2 text-center text-sm text-muted underline">
